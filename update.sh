...
# === Ручной режим обновления (LOCAL-CUSTOMIZATIONS.md §7.1, решение пилота 2026-09-18) ===
# Автообновление шаблона запрещено: установка далеко ушла от вендорской (локальные
# патчи и кастомы), поэтому обновление запускается ТОЛЬКО вручную, осознанно —
# с явной переменной-подтверждением. Любой автоматический или агентный вызов
# отвергается здесь, до любых мутаций. Штатный путь — обёртка, она восстанавливает
# кастомы после обновления.
if [ "${IWE_ALLOW_TEMPLATE_UPDATE:-0}" != "1" ]; then
    echo "⛔ Обновление шаблона IWE отключено: разрешён только ручной запуск." >&2
    echo "   Штатный путь: IWE_ALLOW_TEMPLATE_UPDATE=1 bash /q/IWE/custom/update-with-custom.sh" >&2
    echo "   Правило и откат: DS-strategy/docs/LOCAL-CUSTOMIZATIONS.md §7.1" >&2
    exit 9
fi
