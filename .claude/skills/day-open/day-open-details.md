...
**Обновление IWE — только вручную (реестр §7.1, решение пилота 2026-09-18):**
Day Open НЕ запускает `update.sh` (даже `--check`) — автообновление запрещено. В строке «Update IWE» показывается локальная версия (`.iwe-runtime/.build-hash`) и пометка «обновление отключено — только вручную». Осознанный запуск — из терминала: `IWE_ALLOW_TEMPLATE_UPDATE=1 bash /q/IWE/custom/update-with-custom.sh`.
