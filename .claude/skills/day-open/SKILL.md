...
Автообновление отключено (реестр §7.1): Day Open НЕ запускает `update.sh`. Строка «Update IWE» — локальная версия из `.iwe-runtime/.build-hash` + пометка «обновление отключено — только вручную». Base-репо (FPF, SPF, ZP) проверяются на отставание от origin как раньше. Scout report не проревьюен → «Требует внимания».
...
> Канбан, плановый/фактический мультипликатор и соответствующую подсекцию Week Close.
