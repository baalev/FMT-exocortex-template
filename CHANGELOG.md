...
- `fix(close-wp.sh)` (этот коммит): шаблон замены `\1` + год склеивался в восьмеричную escape-последовательность (`\120` → `P`), из-за чего при **повторном** закрытии РП строка `closed_date: 2026-09-15` превращалась в `P26-09-15` и frontmatter переставал быть валидным YAML. Замена переведена на однозначный `\g<1>`; баг воспроизведён и закрыт проверкой
