...
INCEOF
  ...
  # Обновление шаблона — ТОЛЬКО вручную (LOCAL-CUSTOMIZATIONS.md §7.1, решение
  # пилота 2026-09-18). Автоматический вызов update.sh (в т.ч. --check) убран:
  # скрипт обновления не должен запускаться сам. Статус читается локально,
  # без сети и без запуска обновления.
  if [ -d "$IWE/FMT-exocortex-template" ]; then
    local local_ver
    local_ver=$(grep -m1 '^FMT version:' "$IWE/.iwe-runtime/.build-hash" 2>/dev/null | cut -d' ' -f3)
    echo "| Update IWE | 🔒 | обновление отключено — только вручную (локально ${local_ver:-неизвестно}) |"
  fi
